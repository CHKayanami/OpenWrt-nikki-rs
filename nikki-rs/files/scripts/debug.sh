...
enabled=`uci get nikki-rs.config.enabled`

if [ "$enabled" == "0" ]; then
	uci set nikki-rs.config.enabled=1
	uci commit nikki-rs
	/etc/init.d/nikki-rs restart
fi
...
`
if [ -x "/bin/opkg" ]; then
	opkg list-installed "nikki-rs"
	opkg list-installed "luci-app-nikki-rs"
elif [ -x "/usr/bin/apk" ]; then
	apk list -I "nikki-rs"
	apk list -I "luci-app-nikki-rs"
fi
`
\`\`\`
## config
...
const config = uci.get_all("nikki-rs");
const result = {};
...
TPROXY: 
`
ip route list table "$(uci get nikki-rs.routing.tproxy_route_table)"
`

TUN: 
`
ip route list table "$(uci get nikki-rs.routing.tun_route_table)"
`
\`\`\`
## ip6 rule
\`\`\`
...
TPROXY: 
`
ip -6 route list table "$(uci get nikki-rs.routing.tproxy_route_table)"
`

TUN: 
`
ip -6 route list table "$(uci get nikki-rs.routing.tun_route_table)"
`
\`\`\`
## nftables
\`\`\`
`
nft list table inet nikki-rs
`
\`\`\`
## service
\`\`\`json
`
/etc/init.d/nikki-rs info
`
\`\`\`
"

if [ "$enabled" == "0" ]; then
	uci set nikki-rs.config.enabled=0
	uci commit nikki-rs
	/etc/init.d/nikki-rs restart
fi
