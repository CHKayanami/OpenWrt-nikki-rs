...
DNS_DIR="$HOME_DIR/dns"
BLACK_FILTER_PATH="$DNS_DIR/black_filter"
FAKE_IP_FILTER_PATH="$DNS_DIR/fake_ip_filter"
FALLBACK_FILTER_PATH="$DNS_DIR/fallback_filter"

# log
LOG_DIR="/var/log/nikki-rs"
APP_LOG_PATH="$LOG_DIR/app.log"
CORE_LOG_PATH="$LOG_DIR/core.log"

# temp
TEMP_DIR="/var/run/nikki-rs"
PID_FILE_PATH="$TEMP_DIR/nikki-rs.pid"
...
prepare_files() {
	...
	if [ ! -d "$RULE_PROVIDERS_DIR" ]; then
		mkdir -p "$RULE_PROVIDERS_DIR"
	fi
	if [ ! -d "$PROXY_PROVIDERS_DIR" ]; then
		mkdir -p "$PROXY_PROVIDERS_DIR"
	fi
	if [ ! -d "$DNS_DIR" ]; then
		mkdir -p "$DNS_DIR"
	fi
